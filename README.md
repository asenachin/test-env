...
Считаю, что работа базы данных под нагрузкой и без будет отличаться только временем выполнения запросов.  
Также стоит отметить, что создание индекса не всегда может привести к уменьшению времени выполнения запросов, например, для небольших по размеру таблиц. 
В данном случае, так как время создания индекса в десятки раз превышает время выполнения запроса, то я бы не стал создавать индекс.  
Если индекс необходимо всё же создать, то я бы завершил все процессы с нужным полем таблицы и создал индекс, как в примере выше.  

### Является ли хорошей практикой создание индекса во время высокой транзакционной нагрузки?  

Создание индекса во время высокой транзакционной нагрузки может блокировать данную таблицу базы данных, что может привести к замедлению выполнения транзакций или возможно даже к ошибкам их выполнения. Поэтому я думаю, что для создания индексов лучше будет завершить транзакции с данной таблицей и тогда уже создать индекс.
